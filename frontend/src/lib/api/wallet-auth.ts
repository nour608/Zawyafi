...
export const buildWalletAuthMessage = (address: string, timestamp: string): string =>
  [`Zawyafi API Session`, `address:${address.toLowerCase()}`, `timestamp:${timestamp}`].join('\n')
...
const getCachedSession = (address: string, nowMs: number): WalletAuthSession | null => {
  if (!cachedSession) {
    return null
  }
...
export const getWalletAuthHeaders = async (account?: WalletAuthAccount): Promise<Record<string, string>> => {
