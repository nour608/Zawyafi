...
export function Sidebar() {
  ...
  const links = [
    { name: 'Marketplace', href: '/investor/marketplace', icon: Store },
    { name: 'Portfolio', href: '/investor/portfolio', icon: PieChart },
    { name: 'Wallet', href: '/investor/wallet', icon: Wallet },
    { name: 'Analytics', href: '/investor/analytics', icon: BarChart2 },
  ] as const
    ...
    <aside className="hidden h-full w-72 shrink-0 flex-col border-r border-line bg-panel/80 backdrop-blur lg:flex">
      <div className="p-6 pb-2">
        <Link href="/" className="mb-8 inline-flex flex-col items-start gap-1 transition-opacity hover:opacity-85">
          ...
          <p className="text-[11px] font-semibold uppercase tracking-wider text-textMuted">Private Equity</p>
        </Link>
      ...
      <div className="border-t border-line p-4">
        <div className="flex cursor-pointer items-center gap-3 rounded-lg border border-transparent px-2 py-2 transition-colors hover:border-line hover:bg-panelMuted">
          <div className="flex size-10 items-center justify-center rounded-full border border-line bg-panel font-bold text-gold">AA</div>
          <div className="flex flex-col overflow-hidden">
            <p className="truncate text-sm font-bold text-text">Ahmed Al-Sayed</p>
            <p className="truncate text-xs text-textMuted">Standard Investor</p>
          </div>
        </div>
      </div>
    </aside>
