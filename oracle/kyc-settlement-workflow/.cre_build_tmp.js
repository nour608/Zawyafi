...
var parseApprovedAt = (approvedAt, runtime2) => {
  const parsedMs = Date.parse(approvedAt);
  if (!Number.isFinite(parsedMs)) {
    return BigInt(Math.floor(runtime2.now().getTime() / 1000));
  }
  return BigInt(Math.floor(parsedMs / 1000));
};
var onCronTrigger = (runtime2, _payload) => {
  ...
  const ready = sendJson(runtime2, {
    url: `${runtime2.config.backendBaseUrl}${runtime2.config.readyOnchainPath}?limit=${runtime2.config.maxBatch}`,
    method: "GET",
    headers: {
  ...
  if (!ready.records || ready.records.length === 0) {
    runtime2.log("No approved KYC records ready for settlement");
  ...
  for (const record of ready.records) {
    ...
    const approvedAt = parseApprovedAt(record.approvedAt, runtime2);
    try {
      ...
      const writeResponse = evmClient.writeReport(runtime2, {
        receiver: runtime2.config.kycReceiverAddress,
        report: report2,
        gasConfig: {
      ...
      processed += 1;
      runtime2.log(`KYC settlement success for ${record.requestId}: ${txHash}`);
    } catch (error) {
      sendJson(runtime2, {
        url: `${runtime2.config.backendBaseUrl}${runtime2.config.onchainResultPath}`,
        method: "POST",
