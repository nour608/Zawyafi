...
var configSchema = exports_external.object({
  ...
  category: exports_external.string().describe("Product category for report identity"),
...
var fetchCafeRevenueFromSquare = (nodeRuntime, window, squareToken) => {
  ...
  for (const refund of refunds) {
    if (refund.status !== "COMPLETED")
      continue;
    completedRefunds += 1;
    if (!refund.payment_id || !selectedPaymentIds.has(refund.payment_id))
      continue;
  ...
  const refundRatioBps = grossSales > 0n ? Number(refundAmount * 10000n / grossSales) : 0;
  ...
  const periodId = keccak256(encodeAbiParameters(parseAbiParameters("bytes32, bytes32, uint64, uint64"), [merchantIdHash, productIdHash, periodStart, periodEnd]));
  nodeRuntime.log(`Square node fetch summary: payments_total=${payments.length}, payments_completed=${completedPayments}, payments_selected=${selectedPaymentIds.size}, refunds_total=${refunds.length}, refunds_completed=${completedRefunds}, refunds_matched=${Number(matchedRefundEvents)}`);
  nodeRuntime.log(`Square node computed amounts(cents): gross=${grossSales.toString()}, refunds=${refundAmount.toString()}, net=${netSales.toString()}, units=${unitsSold.toString()}, refundUnits=${refundUnits.toString()}, netUnits=${netUnitsSold.toString()}`);
  nodeRuntime.log(`Anomaly evaluation: refundRatioBps=${refundRatioBps}, refundRatioThresholdBps=${config.anomalyRefundRatioBpsThreshold}, netSales=${netSales.toString()}, netSalesSpikeThreshold=${config.anomalyNetSalesSpikeCentsThreshold.toString()}, flagged=${flagged}, status=${status}, riskScore=${riskScore}`);
  return {
    periodId,
    merchantIdHash,
    productIdHash,
...
var onCronTrigger = (runtime2, payload) => {
  ...
  runtime2.log(`Fetching Square data from ${window.beginIso} to ${window.endIso} for category ${runtime2.config.category}`);
  const report2 = runtime2.runInNodeMode(fetchCafeRevenueFromSquare, ConsensusAggregationByFields({
    periodId: identical,
  ...
  }))(window, squareToken).result();
  runtime2.log(`CRE aggregated report: ${JSON.stringify(toReportLogView(report2))}`);
  if (runtime2.config.skipWrite) {
    runtime2.log("skipWrite=true, not sending report onchain");
    return JSON.stringify({
      skippedWrite: true,
      batchId: runtime2.config.batchId.toString(),
      report: toReportLogView(report2)
    });
  }
  return submitReport(runtime2, report2);
};
async function main() {
