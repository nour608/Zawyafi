...
var configSchema = exports_external.object({
  ...
  readyPath: exports_external.string(),
  resultPath: exports_external.string(),
...
var parseValuesBigInt = (value2) => {
  if (!value2)
    return 0n;
  let magnitude = 0n;
  for (const b of value2.absVal) {
...
var toValuesBigIntJson = (value2) => {
  const sign = value2 < 0n ? "-1" : value2 > 0n ? "1" : "0";
  const magnitude = value2 < 0n ? -value2 : value2;
...
var toIsoDateTime = (seconds) => new Date(Number(seconds) * 1000).toISOString();
...
var buildReportForRecord = (runtime2, evmClient, record) => {
  ...
  for (const log of logs) {
    ...
    const topics = log.topics.map((topic) => bytesToHex(topic).toLowerCase());
    ...
    const blockNumber = parseValuesBigInt(log.blockNumber ?? undefined).toString();
  ...
  for (const [periodId, logMeta] of periodLogMap.entries()) {
    ...
    const callResult = evmClient.callContract(runtime2, {
      call: {
        ...
        to: hexToBase64(runtime2.config.revenueRegistryAddress),
    ...
    const report2 = decoded;
    if (!withinDateWindow(report2.periodStart, report2.periodEnd, record.startDate, record.endDate)) {
      continue;
    }
    periods.push({
      ...
      periodStart: toIsoDateTime(report2.periodStart),
      periodEnd: toIsoDateTime(report2.periodEnd),
      generatedAt: toIsoDateTime(report2.generatedAt),
      grossSalesMinor: report2.grossSales.toString(),
      refundsMinor: report2.refunds.toString(),
      netSalesMinor: report2.netSales.toString(),
      unitsSold: report2.unitsSold.toString(),
      refundUnits: report2.refundUnits.toString(),
      netUnitsSold: report2.netUnitsSold.toString(),
      eventCount: toSafeNumber(report2.eventCount),
      status: Number(report2.status) === 1 ? "VERIFIED" : "UNVERIFIED",
      riskScore: Number(report2.riskScore),
      reasonCode: decodeReasonCode(report2.reasonCode),
