...
var UNVERIFIED_STATUS = 0;
var REASON_OK = stringToHex("OK", { size: 32 });
...
var configSchema = exports_external.object({
  ...
  squareVersion: exports_external.string().describe("Square API Version"),
  locationId: exports_external.string().describe("Square Location ID"),
  merchantId: exports_external.string().describe("Merchant ID for Square"),
  batchId: exports_external.coerce.bigint().describe("Batch ID associated with this revenue"),
  oracleCoordinatorAddress: exports_external.string().describe("Address of the OracleCoordinator contract"),
  chainSelectorName: exports_external.string().describe("Chain selector name for the target network"),
  isTestnet: exports_external.boolean().describe("Whether target chain is testnet"),
  gasLimit: exports_external.string().describe("Gas limit for the transaction"),
  anomalyRefundRatioBpsThreshold: exports_external.number().int().min(1).max(1e4).describe("Refund ratio threshold in basis points to mark period as anomalous"),
  anomalyNetSalesSpikeCentsThreshold: exports_external.coerce.bigint().describe("Absolute net-sales threshold in cents to mark period as sudden-spike anomaly"),
  minEventCountForWrite: exports_external.coerce.number().int().positive().describe("Minimum matched events required before writing"),
  skipWrite: exports_external.boolean().describe("If true, run fetch+consensus only and skip onchain write")
});
var periodReportAndBatchParams = parseAbiParameters("(bytes32 periodId, bytes32 merchantIdHash, bytes32 productIdHash, uint64 periodStart, uint64 periodEnd, uint256 grossSales, uint256 refunds, uint256 netSales, uint256 unitsSold, uint256 refundUnits, uint256 netUnitsSold, uint256 eventCount, bytes32 batchHash, uint64 generatedAt, uint8 status, uint16 riskScore, bytes32 reasonCode) report, uint256 batchId");
var inferCategoryFromNote = (note) => {
  if (!note)
    return;
...
var decodeJsonBody = (body) => {
  const text = new TextDecoder().decode(body);
  return JSON.parse(text);
};
var buildSquareUrl = (baseUrl, endpoint, query) => {
  const normalizedBase = baseUrl.endsWith("/") ? baseUrl.slice(0, -1) : baseUrl;
  const normalizedEndpoint = endpoint.startsWith("/") ? endpoint : `/${endpoint}`;
  const queryParts = [];
  for (const [key, value2] of Object.entries(query)) {
    if (value2 !== undefined) {
      queryParts.push(`${encodeURIComponent(key)}=${encodeURIComponent(value2)}`);
    }
  }
...
var sendSquareGet = (nodeRuntime, httpClient, config, url, squareToken) => {
  if (squareToken) {
  ...
  const confidentialHttpClient = new ClientCapability2;
  let lastError;
  for (const namespace of ["main", ""]) {
    try {
      const response = confidentialHttpClient.sendRequest(nodeRuntime, {
        request: {
          method: "GET",
          url,
          multiHeaders: {
...
var fetchSquarePayments = (nodeRuntime, httpClient, config, window, squareToken) => {
  const payments = [];
  let cursor;
  do {
    ...
    const page = sendSquareGet(nodeRuntime, httpClient, config, url, squareToken);
    payments.push(...page.payments ?? []);
    cursor = page.cursor;
  } while (cursor);
  return payments;
};
var fetchSquareRefunds = (nodeRuntime, httpClient, config, window, squareToken) => {
  const refunds = [];
  let cursor;
  do {
...
var fetchSquareSnapshot = (nodeRuntime, window, squareToken) => {
  ...
  const paymentToCategoryHash = new Map;
  ...
  const categories = Array.from(categoryStats.entries()).map(([categoryHash, category]) => ({
    categoryHash,
  ...
  nodeRuntime.log(`Square snapshot built: payments_total=${payments.length}, payments_completed=${completedPayments}, refunds_total=${refunds.length}, refunds_completed=${completedRefunds}, categories=${categories.length}`);
...
var toReportLogView = (report2) => ({
  periodId: report2.periodId,
  merchantIdHash: report2.merchantIdHash,
  productIdHash: report2.productIdHash,
  periodStart: report2.periodStart.toString(),
  periodEnd: report2.periodEnd.toString(),
  grossSales: report2.grossSales.toString(),
  refunds: report2.refunds.toString(),
  netSales: report2.netSales.toString(),
  unitsSold: report2.unitsSold.toString(),
  refundUnits: report2.refundUnits.toString(),
  netUnitsSold: report2.netUnitsSold.toString(),
  eventCount: report2.eventCount.toString(),
  batchHash: report2.batchHash,
  generatedAt: report2.generatedAt.toString(),
  status: report2.status,
  riskScore: report2.riskScore,
  reasonCode: report2.reasonCode
});
var submitReport = (runtime2, report2) => {
  ...
  const writeResponse = evmClient.writeReport(runtime2, {
    receiver: runtime2.config.oracleCoordinatorAddress,
...
var getScheduledDate = (runtime2, payload) => {
  const rawTimestamp = payload.scheduledExecutionTime;
  if (!rawTimestamp?.seconds) {
    return runtime2.now();
  }
  const seconds = typeof rawTimestamp.seconds === "bigint" ? Number(rawTimestamp.seconds) : rawTimestamp.seconds;
...
var readContract = (runtime2, evmClient, to, abi, functionName, args = []) => {
...
var buildReportsForTokenizedCategories = (runtime2, window, tokenizedCategoryHashes, snapshot) => {
  ...
  for (const categoryHash of tokenizedCategoryHashes) {
    ...
    const refundRatioBps = grossSales > 0n ? Number(refunds * 10000n / grossSales) : 0;
    const refundRatioFlagged = refundRatioBps >= runtime2.config.anomalyRefundRatioBpsThreshold;
    const suddenSpikeFlagged = netSales >= runtime2.config.anomalyNetSalesSpikeCentsThreshold;
    ...
    const periodId = keccak256(encodeAbiParameters(parseAbiParameters("bytes32, bytes32, uint64, uint64"), [merchantIdHash, categoryHash, periodStart, periodEnd]));
    reports.push({
      periodId,
      merchantIdHash,
...
var onCronTrigger = (runtime2, payload) => {
  ...
  const factoryAddress = readContract(runtime2, evmClient, settlementVaultAddress, settlementVaultReadAbi, "factory", []);
  const tokenizedCategoryHashes = readContract(runtime2, evmClient, factoryAddress, factoryReadAbi, "getBatchCategoryHashes", [runtime2.config.batchId]);
  if (!tokenizedCategoryHashes || tokenizedCategoryHashes.length === 0) {
    runtime2.log(`No tokenized categories for batch ${runtime2.config.batchId.toString()}, skipping`);
    return JSON.stringify({
      skippedWrite: true,
  ...
  let squareToken;
  try {
    squareToken = runtime2.getSecret({ id: "SQUARE_PAT", namespace: "main" }).result().value;
  } catch {
    try {
      squareToken = runtime2.getSecret({ id: "SQUARE_PAT" }).result().value;
    } catch {
      squareToken = undefined;
      runtime2.log("runtime.getSecret for SQUARE_PAT failed; using confidential-http secret injection fallback");
    }
  }
  runtime2.log(`Fetching Square data from ${window.beginIso} to ${window.endIso} for batch ${runtime2.config.batchId.toString()}`);
  const snapshotRaw = runtime2.runInNodeMode(fetchSquareSnapshot, consensusIdenticalAggregation())(window, squareToken).result();
  const snapshot = JSON.parse(snapshotRaw);
  const reports = buildReportsForTokenizedCategories(runtime2, window, tokenizedCategoryHashes, snapshot);
  runtime2.log(`Built ${reports.length} report(s) from ${snapshot.categories.length} detected category bucket(s), tokenizedCategories=${tokenizedCategoryHashes.length}`);
  if (reports.length === 0) {
    return JSON.stringify({
      skippedWrite: true,
  ...
  runtime2.log(`CRE aggregated reports: ${JSON.stringify(reports.map((report2) => toReportLogView(report2)))}`);
  if (runtime2.config.skipWrite) {
    runtime2.log("skipWrite=true, not sending reports onchain");
    return JSON.stringify({
      skippedWrite: true,
      batchId: runtime2.config.batchId.toString(),
      reports: reports.map((report2) => toReportLogView(report2))
    });
  }
  const txHashes = [];
